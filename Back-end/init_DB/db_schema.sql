...
CREATE FUNCTION check_admin_permission(arg_person_id INT, arg_home_id INT)
RETURNS BOOLEAN AS
$$
DECLARE
	role domobus.role%rowtype;
BEGIN
	SELECT role_id FROM domobus.home_person_role
...
CREATE FUNCTION check_division_permission(arg_person_id INT, arg_division_id INT)
RETURNS BOOLEAN AS
$$
DECLARE
...
BEGIN
	IF division_id IS NULL THEN
		RETURN TRUE;
	ELSE
		SELECT can_control FROM domobus.person_division_access
...
BEGIN
	...
	INTO device;
	IF check_admin_permission(arg_person_id, device.device_home_id)
	OR check_division_permission(arg_person_id, device.device_division_id) THEN
...
CREATE FUNCTION add_values_for_new_device()
RETURNS TRIGGER AS
$$
BEGIN
